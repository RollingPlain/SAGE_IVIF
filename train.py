...
def process_teacher_batch(data, model_teacher, model_student, optimizer_teacher, scheduler_teacher, 
	...
	if epoch >= args.ini_epoch: 
		...
		if not hasattr(process_teacher_batch, "_seg_model"):
			from xdecoder.infer_semseg import set_sys_args  # [MOD] 延迟导入
			from xdecoder.utils.arguments import load_opt_command  # [MOD]
			from xdecoder.modeling.BaseModel import BaseModel  # [MOD]
			from xdecoder.modeling import build_model  # [MOD]

			set_sys_args()
			opt, _ = load_opt_command([])
			opt['local_rank'] = 0
			dev_type = device if isinstance(device, str) else device.type  # [MOD]
			opt['device'] = dev_type
			...
			seg_model = BaseModel(opt, build_model(opt)).from_pretrained(pretrained_pth).eval().to(device)
			for param in seg_model.parameters():
				param.requires_grad = False  # [MOD] 不更新 xdecoder，仅传递梯度到输入
			seg_model.model.sem_seg_head.predictor.lang_encoder.get_text_embeddings(
				["road", "sidewalk", "building", "lamp", "sign", "vegetation", "sky", "person", "car",
				 "truck", "bus", "motocycle", "bicycle", "pole", "background"], is_eval=True
			)
			seg_model.model.sem_seg_head.num_classes = 14
			process_teacher_batch._seg_model = seg_model
			process_teacher_batch._seg_resize = 384  # [MOD] reduce resolution to save memory

		seg_model = process_teacher_batch._seg_model
		target_hw = label_mask.shape[-2:]

		output_masks = []
		try:
			for j in range(output_colored.size(0)):
				single_image = torch.clamp(output_colored[j:j+1], 0.0, 1.0)
				resized = F.interpolate(
					single_image, size=(process_teacher_batch._seg_resize, process_teacher_batch._seg_resize),
					mode='bicubic', align_corners=False
				) * 255.0  # [MOD] 模拟 transforms.Resize + 0-255 像素

				batch_inputs = [{
					'image': resized.squeeze(0),
					'height': resized.size(2),
					'width': resized.size(3)
				}]
				seg_outputs = seg_model(batch_inputs)
				sem_seg = seg_outputs[-1]['sem_seg']
				if sem_seg.dim() == 3:
					sem_seg = sem_seg.unsqueeze(0)  # [MOD] ensure N,C,H,W for interpolate
				sem_seg = F.interpolate(
					sem_seg, size=target_hw, mode='bilinear', align_corners=False
				)
				output_masks.append(sem_seg)
